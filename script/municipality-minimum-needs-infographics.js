/**
 * Municipality Minimum Needs Infographics
 * Displays minimum needs data (food packs, water, shelter, etc.) 
 * by selected flood extent for the whole municipality
 * Uses AnalysisSummary files (24MetersAnalysisSummary_0.js, etc.)
 * Shows on the right side of the screen when Minimum Needs button is clicked
 * and municipality is selected (but no barangay)
 */
...
(function() {
    ...
    /**
     * Extract minimum needs data from AnalysisSummary for the whole municipality
     * @param {string} floodExtent - Flood extent value
     * @returns {Object} - Extracted minimum needs data
     */
    function extractMinimumNeedsData(floodExtent) {
    ...
    function createNeedsPanel() {
        ...
        needsPanel.id = 'municipality-minimum-needs-panel';
        needsPanel.className = 'minimum-needs-panel';

        // Create panel header
        var panelHeader = document.createElement('div');
        panelHeader.className = 'minimum-needs-header';
        panelHeader.innerHTML = '<h3>Municipality Minimum Needs</h3><button class="minimum-needs-close" id="municipality-minimum-needs-close">&times;</button>';

        // Create panel content
        var panelContent = document.createElement('div');
        panelContent.className = 'minimum-needs-content';
        panelContent.id = 'municipality-minimum-needs-content';
        ...
        var closeBtn = document.getElementById('municipality-minimum-needs-close');
        if (closeBtn) {
     ...
     */
    function showPanel() {
        // Hide barangay panel if it's active
        var barangayPanel = document.getElementById('minimum-needs-panel');
        if (barangayPanel && barangayPanel.classList.contains('active')) {
            barangayPanel.classList.remove('active');
        }
        ...
        var panelContent = document.getElementById('municipality-minimum-needs-content');
        if (!panelContent) {
            return;
        }
        ...
        if (selectedMunicipalities.length === 0) {
            ...
            noDataMsg.textContent = 'Please select at least one municipality from the dropdown.';
        ...
        // Extract minimum needs data
        var needsData = extractMinimumNeedsData(selectedFloodExtent);
        ...
        if (categoriesWithData > 0) {
            ...
            var municipalityItem = document.createElement('div');
            municipalityItem.className = 'needs-summary-item';
            var municipalityName = selectedMunicipalities.length > 0 ? selectedMunicipalities[0] : 'Municipality';
            municipalityItem.innerHTML = '<span class="summary-label">Municipality:</span><span class="summary-value">' + municipalityName + '</span>';
            summaryContent.appendChild(municipalityItem);
        ...
        if (grid.children.length === 0) {
            var noDataMsg = document.createElement('div');
            noDataMsg.className = 'needs-no-data';
            noDataMsg.innerHTML = '<i class="fas fa-info-circle" style="font-size: 48px; color: #ccc; margin-bottom: 15px; display: block;"></i>No minimum needs data available for the municipality at <strong>' + selectedFloodExtent + ' meters</strong>.';
            panelContent.appendChild(noDataMsg);
        } else {
    ...
    /**
     * Handle Minimum Needs button click for municipality level
     */
    function handleNeedsButtonClick() {
        ...
        // Only handle municipality level if municipality is selected but no barangay
        if (selectedMunicipalities.length > 0 && selectedBarangays.length === 0) {
            // Toggle panel
            if (isPanelActive) {
                hidePanel();
            } else {
                // Only show if flood extent is selected AND household button is active
                if (selectedFloodExtent && householdActive) {
                    showPanel();
                } else {
    ...
    function handleFloodExtentChange() {
        if (isPanelActive) {
            ...
            // Only show if all conditions are met (municipality selected, no barangay)
            if (selectedFloodExtent && selectedMunicipalities.length > 0 && selectedBarangays.length === 0 && householdActive) {
                showPanel();
            } else {
                hidePanel();
            }
    ...
    /**
     * Initialize municipality minimum needs functionality
     */
    function init() {
        // Listen for Minimum Needs button click - use capture phase to check municipality level first
        var needsBtn = document.querySelector('.needs-btn');
        if (needsBtn) {
            needsBtn.addEventListener('click', function(e) {
                ...
                // Only handle municipality level if municipality is selected and no barangay
                if (selectedMunicipalities.length > 0 && selectedBarangays.length === 0) {
                    e.preventDefault();
                    handleNeedsButtonClick();
                    e.stopImmediatePropagation(); // Prevent barangay handler from firing
                }
        ...
        if (municipalityContainer) {
            municipalityContainer.addEventListener('change', function() {
                if (isPanelActive) {
                    var selectedMunicipalities = getSelectedMunicipalities();
                    var selectedBarangays = getSelectedBarangays();
                    // Only update if municipality is selected and no barangay
                    if (selectedMunicipalities.length > 0 && selectedBarangays.length === 0) {
                        handleFloodExtentChange();
                    } else {
                        hidePanel();
        ...
        // Listen for barangay selection changes - hide panel if barangay is selected
        ...
        if (barangayContainer) {
            barangayContainer.addEventListener('change', function() {
                var selectedBarangays = getSelectedBarangays();
                if (selectedBarangays.length > 0 && isPanelActive) {
                    hidePanel();
                }
            });
        }
        ...
        console.log('Municipality minimum needs infographics functionality initialized');
    }
